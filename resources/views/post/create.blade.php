...
<div class="container">
    <form id="contact" action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <img src="{{ asset('img/create/user.png') }}" alt="Profile Picture" class="profile-picture">

          <fieldset>
            <br><label><h5><b>標題</b></h5></label>
            <input placeholder="請輸入標題" type="text" name="title" required autofocus>
          </fieldset>
          <fieldset>
            <label><h5><b>敘述</b></h5></label>
            <textarea placeholder="請輸入文章內容" type="text" name="content" required></textarea>
          </fieldset>
